...
class User:
    ...
    def addContact(self, contact):
        res = False
        print([x[1] for x in self.getContacts()])
        for user in User.getUsersList():
            if int(contact) == user['userid'] and int(contact) not in [x[1] for x in self.getContacts()]:
            ...
            elif int(contact) in [x[1] for x in self.getContacts()]:
                res = "Пользователь {} уже был добавлен в список контактов пользователя {}".format(user['name'], self.name)
            else:
    ...
    def getContacts(self):
        ...
        if res:
            ...
            contacts ="Список контактов пользователя {} пуст".format(self.name)
        conn.commit()
        conn.close()
        return contacts
